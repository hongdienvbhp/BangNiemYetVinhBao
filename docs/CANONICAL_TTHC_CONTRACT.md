...
- `source_commit`: fingerprint SHA-1 xác định nội dung của **bundle nguồn đầu vào đã được kiểm chứng dùng để dựng dataset** (`source_commit_kind=verified_source_bundle_git_sha1`), không phải SHA của commit đang chứa `data/thu-tuc.json`.
...
Mỗi trường có nội dung trong enrichment phải có `fieldProvenance` trỏ tới `sources[].id`; vai trò của từng nguồn nằm ở `sources[].sourceRole`. Riêng `submissionUrl` chỉ được provenance từ nguồn có `sourceRole=local_execution` và phải đúng:
